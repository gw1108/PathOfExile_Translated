...
test('invariants: no empty values, no English substituted into other locales, NFC output', async () => {
  ...
  // Terms with no English reference are dropped (dev filler), only reported.
  assert.ok(!('base_items.Metadata/Items/OnlyInSpanish' in es));
...
test('exportRun writes sorted UTF-8 files without BOM plus index.json with provenance', async () => {
  ...
  assert.deepEqual(summary.localeCounts, { en: 6, es: 5 });

  const buf = await readFile(join(distRoot, 'poe1', 'full', 'es.json'));
  ...
  assert.ok(buf.toString('utf8').includes('Frío'), 'non-ASCII must be raw UTF-8, not \\u-escaped');
...
test('poe2 golden: game-subdir run ingests markup-stripped, placeholder-free dictionaries', async () => {
  ...
  assert.deepEqual(buildLocaleExport(result, 'en'), {
    ...
    'passives.mana_regeneration22_': 'Open Mind',
    ...
    'stat_translations.trigger_interval_stat[0]': '# second between Triggers',
    'stat_translations.trigger_interval_stat[1]': '# seconds between Triggers',
    'stat_translations.unique_jewel_grants_x_voices_jewel_sockets[0]': 'Allocates # Sinister Jewel sockets',
    'stat_translations.unique_jewel_grants_x_voices_jewel_sockets[1]': 'Allocates # Sinister Jewel socket',
  ...
  assert.deepEqual(buildLocaleExport(result, 'ja'), {
    ...
    'passives.mana_regeneration22_': 'オープンマインド',
    'skill_gems.Metadata/Items/Gem/SkillGemFireball': 'ファイアボール',
    'skill_gems.Metadata/Items/Gems/SupportGemMartialTempo': 'マーシャルテンポ',
    'stat_translations.unique_jewel_grants_x_voices_jewel_sockets[0]': '#個のシニスターなジュエルソケットを割り当てる',
    'stat_translations.unique_jewel_grants_x_voices_jewel_sockets[1]': '#個のシニスターなジュエルソケットを割り当てる(単数)',
  ...
  // Variant-alignment guard: ja has 1 variant for trigger_interval_stat where
  // English has 2 — positional pairing is unsafe, so ja drops the entry.
  const ja = buildLocaleExport(result, 'ja');
  assert.ok(!('stat_translations.trigger_interval_stat[0]' in ja));
  assert.ok(!('stat_translations.trigger_interval_stat[1]' in ja));
  const jaSt = result.report.details.find((d) => d.locale === 'ja' && d.namespace === 'stat_translations');
  assert.ok(jaSt?.notes.some((n) => /dropped 1 entries whose variant count differs/.test(n)));
});
...
test('poe2 export: dist/poe2 with game-scoped index.json and provenance', async () => {
  ...
  assert.deepEqual(summary.localeCounts, { en: 12, ja: 10, th: 1 });
  ...
  assert.equal(index.namespaces.base_items, 1);
  assert.equal(index.namespaces.stat_translations, 4);
  assert.equal(index.namespaces.passives, 1);
  assert.equal(index.locales.ja.file, 'full/ja.json');
  ...
  // Per-namespace slices use bare term ids; full/ uses namespace.term_id.
  const jaUniques = JSON.parse(await readFile(join(distRoot, 'poe2', 'uniques', 'ja.json'), 'utf8'));
  assert.deepEqual(jaUniques, { Bramblejack: 'ブランブルジャック' });
  const jaSt = JSON.parse(await readFile(join(distRoot, 'poe2', 'stat_translations', 'ja.json'), 'utf8'));
  assert.equal(jaSt['unique_jewel_grants_x_voices_jewel_sockets[0]'], '#個のシニスターなジュエルソケットを割り当てる');
  const jaFull = JSON.parse(await readFile(join(distRoot, 'poe2', 'full', 'ja.json'), 'utf8'));
  assert.equal(jaFull['uniques.Bramblejack'], 'ブランブルジャック');
});
